...
class RSA:
    def __init__(self, s=64):
        ...
        self.currentDir = re.sub(r"/[^/]*\.py$", "", __file__)
    ...
    def encrypt(self, textFile):
        ...
        if not os.path.exists(f"/encryption-keys/{file_name}/"):
            os.makedirs(
                os.path.dirname(f"{self.currentDir}/encryption-keys/{file_name}/")
            )
        ...
        encrypted_pub_k.write(
            f'{("-" * 15)} Public Key {("-" * 15)} \n{hex(self.key[0][0])}'
        )
        encrypted_priv_k.write(
            f'{("-" * 15)} Priivate Key {("-" * 15)} \n{hex(self.key[1][0])}'
        )
    ...
    def decrypt(self, enc_file=None):
        ...
        decrypted_f = open(f"{self.currentDir}/{file_name}-decr.{ext}", "w+")
        ...
        res = [
            chr(self.squareMultiply(eval("0x" + i), eval(priv_key), eval(pub_key)))
            for i in f.split(" ")
        ]
...
if __name__ in ["__main__", "__live_coding__"]:
    ...
    if res == "1":
        ...
        path = f"{text}"
        assert os.path.exists(f"{currentDir}/{path}"), "No file found!"

        # Run the encryption schema on the text file.
        cipher = rsa.encrypt(path)
